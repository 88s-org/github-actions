# The 9 Contexts in GitHub Actions
...
> **Key gotcha:** `needs` is only available at the **job level** `if:` — you can't use it inside a step's `if:`. Use `needs` outputs via `env:` to bring values into steps.
